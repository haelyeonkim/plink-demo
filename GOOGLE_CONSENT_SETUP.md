...
2. 사이트 배포 후 로그인하지 않은 상태에서 `https://plink.ddak.app/privacy`가 열리는지 확인합니다.
3. Google Cloud Console → Google Auth Platform → Branding에서 앱 링크를 설정합니다.
   - 홈페이지: `https://plink.ddak.app/`
   - 개인정보처리방침: `https://plink.ddak.app/privacy`
...
OAuth 콜백 주소는 기존 `https://plink.ddak.app/login/oauth2/code/google`을 유지합니다. 개인정보처리방침 주소를 콜백 주소로 넣지 않습니다.
