...
- **입장권(Phase 0-2)**: 개인 URL 발급·메일 발송, 이메일 OTP 확인, URL당 패스키 1개 바인딩, 회전형 QR 제시, 게이트 단말 스캔, 재입장 원장과 회차 정책, 양도, 주최자 콘솔
...
얼굴 인식(Phase 3), 위치 바인딩과 오프라인 열화 모드(Phase 4)는 설계만 되어 있고 구현되지 않았습니다. 메일은 `spring.mail.host`가 없으면 발송되지 않고 경고와 함께 로그로만 남습니다.
