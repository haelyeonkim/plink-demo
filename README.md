...
원본 URL에 만료 시간과 최대 열람 수를 설정하고 수신자별 공유 코드로 전달합니다. 관리 화면에서 생성한 링크와 열람자 이름·열람 시간을 확인할 수 있습니다.

## 구현 현황

- 보호 링크 생성 및 공유 코드 복사
- 링크 목록, 상세 조회 및 삭제
- 이메일·전화번호 수신자 형식 검증 및 수신자별 보호 링크 일괄 생성
- 만료 시간·최대 열람 수 검증
- 열람자 이름과 열람 시각 기록
- Google 로그인, 로그인 상태·이름 표시 및 로그아웃
- 최초 수신자 연락처 확인 후 패스키 등록 및 등록된 패스키만 재접근 허용
...
1. `http://localhost:3000/create`에서 원본 URL, 만료 기간(일), 최대 열람 수와 수신자 이메일·전화번호를 입력합니다. 기본 만료 기간은 7일입니다.
2. 수신자마다 별도의 보호 링크가 생성되므로 각 링크를 해당 수신자에게 전달합니다.
3. 생성된 공유 코드를 복사하고 `/s/공유코드`에 접속하거나 홈에서 코드를 입력합니다.
4. 최초 접속 시 링크를 전달받은 이메일 또는 전화번호를 입력하고 패스키를 등록합니다.
5. `/manage`에서 해당 링크의 상세 화면을 열어 열람 기록을 확인합니다.

패스키 수신 흐름에서는 수신자가 링크 화면에서 연락처를 한 번 확인한 뒤 **패스키 등록하고 수신 확정**을 누릅니다. 등록이 완료된 첫 번째 패스키가 링크에 귀속되고, 이후에는 등록된 패스키만 사용할 수 있습니다. 다른 기기에서 수신자 등록을 다시 진행할 수 없습니다.
...
Spring Security의 OAuth 2.0 / OpenID Connect 로그인으로 Google 인증 결과를 백엔드에서 검증하고 서버 세션을 생성합니다. 로그인 후 `/manage`로 이동하며 헤더에 이름과 로그아웃 버튼이 표시됩니다. 계정 선택 화면을 매번 표시하도록 `prompt=select_account`를 사용합니다. 토큰과 Client Secret은 프론트엔드에 전달하지 않습니다.
...
- 링크 생성 화면에는 비밀번호 보호를 제공하지 않습니다. 링크 접근은 수신자 연락처의 최초 확인과 패스키 등록으로 보호합니다.
...
| POST | /api/links/batch | 수신자별 보호 링크 일괄 생성 |
| DELETE | /api/links/{id} | 링크 삭제 |
| GET | /api/links/s/{code} | 공유 코드로 링크 접근 정보 |
| POST | /api/links/s/{code}/passkey/options | 수신자 확인 및 패스키 인증 옵션 생성 |
| POST | /api/links/s/{code}/passkey/finish | 패스키 등록·인증 완료 및 열람 기록 |
